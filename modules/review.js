...
// deterministically by Arizona date so it's stable all day. Selection lives
// in refresherCore.mjs so the daily email surfaces the same question.
// Returns { question, lesson, correctText } or null when nothing's completed yet.
export async function questionOfTheDay() {
  const lessons = await store.getAll('lessons');
  return questionOfTheDayCore(lessons, dayString());
}
